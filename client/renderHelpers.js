import { addNewPlayer, fetchAllPlayers } from './ajaxHelpers';
...
export const renderAllPlayers = (playerList) => {
  ...
  for (let i = 0; i < detailButtons.length; i++) {
    ...
    button.addEventListener('click', async (event) => {
       const specPlayer = await fetchSinglePlayer(event.target.dataset.id);
       renderSinglePlayer(specPlayer);
    });
  }
};
...
export const renderSinglePlayer = (playerObj) => {
  ...
  let pupHTML = `
    ...
    playerObj.name
      ...
      <button id="see-all" data-id=${pup.id}>Back to all players</button>
    </div>
  ...
  seeAllBtn.addEventListener('click',async () =>{
    const back = await fetchAllPlayers(button.dataset.id)
    renderAllPlayers(back)
  });
};
